...
7. **Verificación de tipos desactivada en build.** `next.config.ts` tiene `typescript: { ignoreBuildErrors: true }` y `package.json` compila con `next build --no-lint`. `strict: true` en `tsconfig.json` queda anulado en la práctica: **el build no atrapa errores de tipo**. Riesgo alto para un agente de IA que asuma que "compila = correcto". *Recomendación:* correr `npx tsc --noEmit` manualmente antes de dar por cerrado cualquier cambio.
   - *Actualización 2026-08-24:* pasada de legibilidad (rama `chore/buenas-practicas-legibilidad`) eliminó casi todo el `any` explícito y los `catch (e: any)` fuera de las rutas huérfanas del punto 20, y de paso migró los 4 `<img>` restantes a `next/image`. `npm run lint` bajó de 134 errores/47 warnings a **8 errores / 0 warnings** (ver detalle en el punto 20 y en `docs/trello-buenas-practicas-legibilidad.md`).
   - *Revisión 2026-09-01:* recuento de `npx tsc --noEmit` corregido — son **19 errores preexistentes**, no 13 (el número anterior estaba mal contado, no es que hayan aumentado; ninguno se tocó en la pasada de legibilidad). Se reparten así: 11 en las rutas huérfanas del punto 20 (columnas de schema desincronizadas), 3 de tipos de `framer-motion` en `MissionVisionValues.tsx`, 1 de `Headers` mockeado en `lib/privacy/parental.ts`, y dos categorías que no estaban documentadas — ver puntos 21 y 22.
...
21. **`GET`/`DELETE /api/admin/compras/[id]` usan un modelo Prisma que no existe: `prisma.compraEntrada`.** El schema actual llama al modelo `Compra` (§10 lista los modelos). A diferencia del punto 20, esta ruta **no está huérfana**: `components/admin/compras/ComprasTable.tsx` sí la llama (`DELETE` al presionar "eliminar compra"), así que **el botón de eliminar compra en el panel admin devuelve un error 500 hoy**. Detectado el 2026-09-01 vía `npx tsc --noEmit` (`error TS2339: Property 'compraEntrada' does not exist`). No se corrigió porque implica decidir el comportamiento correcto (¿debería además borrar los `CompraItem` asociados?), no solo tipar — pero por su severidad (bug activo, no deuda técnica latente) conviene priorizarlo antes que el punto 20.
22. **`app/admin/publicaciones/actions.ts` (líneas 95 y 144) usa `e.errors[0]?.message` sobre un `z.ZodError`**, propiedad que no existe en la versión de Zod instalada (es `.issues`, no `.errors`). Mismo bug de fondo que se encontró y se dejó igual a propósito en otros archivos durante la pasada de legibilidad del 2026-08-24 (ver Tarjeta 1 en `docs/trello-buenas-practicas-legibilidad.md`) — la validación de Zod nunca muestra el mensaje específico del campo, siempre cae al mensaje genérico de `catch`. Este archivo no estaba en el alcance de esa pasada. Baja severidad (el usuario igual ve un mensaje de error, solo que menos específico), pero es la misma corrección mecánica que ya se aplicó en el resto del código.
